...
const SubMultifileFiles = React.memo((props) => {
    const [documents, setDocuments] = useState([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    const [expandedItems, setExpandedItems] = useState([`submultifile-root-${props.item?.id}-${props.item?.classId ?? props.item?.classID}`]);
    ...
    const documentItems = useMemo(() => {
        return documents.map((doc, index) => {
            ...
            const itemId = `submultifile-${props.item?.id}-${doc.fileID}-${index}`;

            return (
                <TreeItem
                    key={`sub-${doc.fileID || index}`}
                    itemId={itemId}
                    onClick={() => {
    ...
    return (
        <>
            ...
            >
                <TreeItem
                    itemId={`submultifile-root-${props.item?.id}-${props.item?.classId ?? props.item?.classID}`}
                    sx={{
                        ...treeItemStyles,
                    }}
                    label={
...
SubMultifileFiles.displayName = 'SubMultifileFiles';

export default SubMultifileFiles;