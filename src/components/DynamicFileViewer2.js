...
const CSVViewer = React.memo(({ csvString, setUploadedFile }) => {
    ...
    const data = useMemo(() => parseCSV(csvString), [csvString]);
...
const DynamicFileViewer = ({ base64Content, fileExtension, setUploadedFile }) => {
    ...
    }, [base64Content, fileExtension, handleViewFile]);
    ...
    const renderContent = useCallback(() => {
        ...
        if (ext === 'txt') {
            return <TextViewer content={atob(base64Content)} setUploadedFile={setUploadedFile} />;
        }
        ...
        if (ext === 'csv') {
            return <CSVViewer csvString={atob(base64Content)} setUploadedFile={setUploadedFile} />;
        }

        return <p>Unsupported file type. {ext}</p>;
    }, [base64Content, fileExtension, fileUrl, setUploadedFile, csvContent]);
