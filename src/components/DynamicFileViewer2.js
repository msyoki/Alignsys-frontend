...
// Optimized ImageViewer with enhanced memoization
const ImageViewer = React.memo(({ src, setUploadedFile }) => {
...
const TextViewer = React.memo(({ content, setUploadedFile }) => {
...
// Optimized CSVViewer with debounced search
const CSVViewer = React.memo(({ csvString, setUploadedFile }) => {
...
// Custom session state hook with optimized performance
...
const DynamicFileViewer = ({ base64Content, fileExtension, setUploadedFile }) => {
  ...
  // Memoized delete handler for office files
  ...
  const renderContent = useMemo(() => {
    ...
    switch (fileType) {
      case 'image':
        return <ImageViewer src={fileUrl} setUploadedFile={setUploadedFile} />;
      case 'pdf':
        return <PDFViewerPreview document={fileUrl} setUploadedFile={setUploadedFile} />;
      case 'text':
        return <TextViewer content={decodedContent} setUploadedFile={setUploadedFile} />;
      case 'office':
        return (
  ...
  }, [base64Content, fileExtension, fileType, fileUrl, isProcessing, decodedContent, setUploadedFile, handleOfficeDelete]);
